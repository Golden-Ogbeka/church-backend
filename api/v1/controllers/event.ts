...
export default () => {
  ...
  const AddEvent = async (
    ...
    try {
      ...
      if (!posterFile) return res.status(404).json({ message: 'No poster uploaded' });
      ...
      if (allowRegistration && !requiredRegistrationDetails?.length)
        return res.status(400).json({
          message: 'Please input the required registration details for this event',
        });
      ...
      if (detailsError.length > 0) {
        return res
          .status(400)
          .json({ message: 'Name and type is required for each registration detail' });
      }
  ...
  const RegisterEvent = async (
    ...
    try {
      ...
      if (!eventRegistrationCheck(event).status)
        return res.status(404).json({ message: eventRegistrationCheck(event).message });
      let regDetails: any = {};
      let regErrors: any = [];
